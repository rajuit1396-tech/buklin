...
void main() {
  ...
  testWidgets(
      'loading vehicle is required and the form preserves page selections',
      (tester) async {
    ...
    final continueButton =
        find.widgetWithText(FilledButton, 'Next: work location');
    ...
    final amount =
        find.widgetWithText(TextFormField, 'Amount you want to give');
    ...
    await tester.scrollUntilVisible(find.text('Search for an operator'), 250,
        scrollable: find.byType(Scrollable).first);
    await tester.scrollUntilVisible(
        find.widgetWithText(TextFormField, 'Store number'), -250,
        scrollable: find.byType(Scrollable).first);
    final store = find.widgetWithText(TextFormField, 'Store number');
    expect(tester.widget<TextFormField>(store).initialValue, '007');
    expect(
        tester
            .widget<TextField>(
                find.descendant(of: store, matching: find.byType(TextField)))
            .readOnly,
        isTrue);
