...
function accountRow(user){
  ...
  const actions=document.createElement('div');actions.className='actions';const adjust=document.createElement('button');adjust.type='button';adjust.className='ghost';adjust.textContent='Adjust balance';adjust.addEventListener('click',()=>adjustBalance(user));actions.append(adjust);
...
async function refresh(){
  try{const search=byId('searchInput').value.trim();const [dashboard,accounts]=await Promise.all([api('/admin/dashboard'),api(`/admin/users?search=${encodeURIComponent(search)}`)]);
    const totals=byId('totals');totals.replaceChildren(stat('Customers',dashboard.totals.customers),stat('Operators',dashboard.totals.operators),stat('Active jobs',dashboard.totals.active_jobs),stat('Completed jobs',dashboard.totals.completed_jobs),stat('Work fees',dashboard.totals.fees),stat('Payments received',dashboard.totals.received),stat('Amount owed',dashboard.totals.owed),stat('Account credit',dashboard.totals.credit));
...
async function adjustBalance(user){
  const raw=prompt(`Enter a whole-number adjustment for ${user.name}. Positive adds credit; negative adds debt.`);if(raw===null)return;const amount=Number(raw);if(!Number.isInteger(amount)||amount===0){notice('Enter a non-zero whole number.',true);return;}
  const reason=prompt('Reason for this adjustment:');if(reason===null)return;
  try{await api(`/admin/users/${user.id}/balance`,{method:'POST',body:JSON.stringify({id:crypto.randomUUID(),amount,reason,kind:'adjustment'})});notice('Balance updated.');await refresh();}catch(error){notice(error.message,true);}
}
...
byId('roleSelect').addEventListener('change',event=>byId('serviceField').hidden=event.target.value!=='operator');
