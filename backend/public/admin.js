...
async function showJobs(user){try{const data=await api(`/admin/users/${user.id}/jobs`);const lines=data.jobs.slice(0,12).map(job=>`${new Date(job.created_at).toLocaleDateString()} · ${job.service} · ${job.status} · ${job.participation}`);alert(`${user.name}\nCompleted: ${data.completed}\nTotal work records: ${data.total}\n\n${lines.join('\n')||'No work history'}`);}catch(error){notice(error.message,true);}}
