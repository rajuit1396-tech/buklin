...
// databases so 3-riyal completion fees work immediately after deployment.
// Retain -4 for historical charges.
await pool.query(`alter table work_charges drop constraint if exists work_charges_amount_check;
  alter table work_charges add constraint work_charges_amount_check check(amount in (-4,-3,-2));
